...
async function startup() {
  ...
  for (var i = 0; i < cardsArr.length; i++) {
    ...
    cardBody.className = 'card-body card-bandori active-card';
    ...
    cardBody.addEventListener('click', function(event) {
      ...
      localStorage.setItem('cd', JSON.stringify(cardsMatch));

    });
  }

  var savedCards = JSON.parse(localStorage.getItem('cd'));
